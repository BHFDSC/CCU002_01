...
fit_get_data_surv <- function(covars, pheno_str, agegp, event, survival_data, cuts_weeks_since_expo){
  ...
  # head(cohort_agegp %>% dplyr::select(!c("SEX", "AGE_AT_COHORT_START", "region_name", "name")) %>% filter(is.na(DATE_EXPO_CENSOR)))
  # head(cohort_agegp %>% dplyr::select(!c("SEX", "AGE_AT_COHORT_START", "region_name", "name")) %>% filter(!is.na(DATE_EXPO_CENSOR)))
  # head(cohort_agegp %>% dplyr::select(!c("SEX", "AGE_AT_COHORT_START", "region_name", "name")) %>% filter(DATE_EXPO_CENSOR==end_date))
  ...
  with_expo %>% 
    dplyr::select(NHS_NUMBER_DEID, expo_date, end_date, event_date, days_to_end, DATE_OF_DEATH, DATE_EXPO_CENSOR) %>%  
  ...
  if (identical(cuts_weeks_since_expo, c(4, 49))){
    write.csv(tbl_event_count, paste0("tbl_event_count_red_" , expo, "_", event, "_", agegp, "_", pheno_str, ".csv"), row.names = T)
  } else (
    write.csv(tbl_event_count, paste0("tbl_event_count_" , expo, "_", event, "_", agegp, "_", pheno_str, ".csv"), row.names = T)
  )
