...
## Author: Samantha Ip
## =============================================================================
source(file.path(scripts_dir,"si_fit_get_data_surv_eventcountbasedtimecuts_covidpheno.R"))
...
coxfit_bkwdselection <- function(data_surv, sex, interval_names, fixed_covars, pheno_str, event, agegp, sex_as_interaction, covar_names){
  cat("...... sex ", sex, " ...... \n")
  ...
  if (sex_as_interaction){
    ...
    saveRDS(fit_red, paste0("fit_", event, "_", agegp, "_", pheno_str, ".rds"))
  }
...
fit_model_reducedcovariates <- function(sex_as_interaction, covars, pheno_str, agegp, event, survival_data){
  list_data_surv_noncase_ids_interval_names <- fit_get_data_surv(covars, pheno_str, agegp, event, survival_data, cuts_weeks_since_expo)
  ...
  if (ind_any_zeroeventperiod){
    ...
    list_data_surv_noncase_ids_interval_names <- fit_get_data_surv(covars, pheno_str, agegp, event, survival_data, cuts_weeks_since_expo_reduced)
    data_surv <- list_data_surv_noncase_ids_interval_names[[1]]
    noncase_ids <- list_data_surv_noncase_ids_interval_names[[2]]
    interval_names <-list_data_surv_noncase_ids_interval_names[[3]]
    ind_any_zeroeventperiod <- list_data_surv_noncase_ids_interval_names[[4]]
  ...
  if (sex_as_interaction){
    # ---------------------  *sex as interaction term  -------------------------
    levels(data_surv$IMD) <- c("Deciles_1_2", "Deciles_3_4", "Deciles_5_6", "Deciles_7_8", "Deciles_9_10", "Deciles_9_10")
    fit <- coxfit_bkwdselection(data_surv, "all", interval_names, fixed_covars, pheno_str, event, agegp, sex_as_interaction, covar_names)
  } else {
    # ------------------------------  + sex  -----------------------------------
    fit_sexall <- coxfit_bkwdselection(data_surv, "all", interval_names, fixed_covars, pheno_str, event, agegp, sex_as_interaction, covar_names)
    # fit_sex1 <- coxfit_bkwdselection(data_surv %>% filter(SEX==1), "1", interval_names, pheno_str, fixed_covars, event, agegp, sex_as_interaction, covar_names)
    # fit_sex2 <- coxfit_bkwdselection(data_surv %>% filter(SEX==2), "2", interval_names, pheno_str, fixed_covars, event, agegp, sex_as_interaction, covar_names)
  ...
  write.csv(fit, paste0("tbl_hr_" , expo, "_", event, "_", agegp, "_", pheno_str, ".csv"), row.names = T)
