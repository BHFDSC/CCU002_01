...
# About:        Fully adjusted model with COVID phenotype, using predefined outcome-specific fixed covariates
# ******************************************************************************
source(file.path(scripts_dir,"fit_get_data_surv_eventcountbasedtimecuts_covidpheno.R"))
...
fit_model_reducedcovariates <- function(sex_as_interaction, covars, pheno_str, agegp, event, survival_data){
  ...
  cat("... fit ... \n")
  print(fit)
  write.csv(fit, paste0("CCU002_DataPreparation/results_phen/tbl_hr_" , expo, "_", event, "_", agegp, "_", pheno_str, ".csv"), row.names = T)
}
#------------------------------------------------------------------------------
