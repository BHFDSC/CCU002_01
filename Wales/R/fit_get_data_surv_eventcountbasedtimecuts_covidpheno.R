...
# About:        Make survival data (with COVID phenotype) -- format for time-dependent coxph
# ******************************************************************************

fit_get_data_surv <- function(covars, pheno_str, agegp, event, survival_data, cuts_weeks_since_expo){
  ...
  if (any(survival_data$DATE_OF_DEATH< cohort_start_date, na.rm=TRUE) |
      ...
      any(survival_data$DATE_EXPO_CENSOR < cohort_start_date, na.rm=TRUE)){
  ...
  if (any(survival_data$DATE_OF_DEATH> cohort_end_date, na.rm=TRUE) |
      ...
      any(survival_data$DATE_EXPO_CENSOR> cohort_end_date, na.rm=TRUE) ){
  ...
  if (!is.na(noncase_frac)){
    cases <- cohort_agegp %>% filter((!is.na(event_date)) &
                                       (
                                         ((event_date <= DATE_OF_DEATH) | is.na(DATE_OF_DEATH)) & ((event_date < DATE_EXPO_CENSOR) | is.na(DATE_EXPO_CENSOR))
                                       ))

    non_cases_exposed <- cohort_agegp %>% filter((!ALF_E %in% cases$ALF_E) & (!is.na(expo_date)))
    non_cases_unexposed <- cohort_agegp %>% filter((!ALF_E %in% cases$ALF_E) & (is.na(expo_date)))
  ...
  system.time(cohort_agegp <-  transform(cohort_agegp, end_date = pmin(event_date, DATE_OF_DEATH, DATE_EXPO_CENSOR, cohort_end_date, na.rm=TRUE)))
  ...
  cohort_agegp$days_to_end <- ifelse((!is.na(cohort_agegp$DATE_EXPO_CENSOR)) & (cohort_agegp$end_date == cohort_agegp$DATE_EXPO_CENSOR), cohort_agegp$days_to_end, (cohort_agegp$days_to_end +1 ))

  noncase_ids <- unique(non_cases_unexposed$ALF_E)
  print("cohort_agegp done")
  head(cohort_agegp )
  #===============================================================================
  #   CACHE some features
  #-------------------------------------------------------------------------------  
  df_sex <- cohort_agegp %>% dplyr::select(ALF_E, SEX)
  df_age_region <- cohort_agegp %>% dplyr::select(ALF_E, AGE_AT_COHORT_START) %>% rename(age = AGE_AT_COHORT_START)
  ...
  with_expo %>%
    dplyr::select(ALF_E, expo_date, end_date, event_date, days_to_end, DATE_OF_DEATH, DATE_EXPO_CENSOR) %>%
    mutate(event_status = if_else( (!is.na(event_date)) &
                                     (
                                       ((event_date <= end_date) & ((end_date != DATE_EXPO_CENSOR) | is.na(DATE_EXPO_CENSOR ))) |
                                         ((event_date < end_date) & (end_date == DATE_EXPO_CENSOR))
                                     ),
                                   1, 0)) -> with_expo
  ...
  d1 <- with_expo %>% dplyr::select(ALF_E, expo_date, event_date, DATE_OF_DEATH)
  d2 <- with_expo %>% dplyr::select(ALF_E, day_expo, days_to_end, event_status)

  with_expo <- tmerge(data1=d1, data2=d2, id=ALF_E,
  ...
  with_expo  <- with_expo %>%
    group_by(ALF_E) %>% arrange(weeks_cat) %>% mutate(last_step = ifelse(row_number()==n(),1,0))
  ...
  data_surv %>% filter(expo_date==event_date) %>% arrange(ALF_E)%>% head(10) %>% print(n = Inf)
  ...
  print("tbl_event_count_all done")
  ...
  if (identical(cuts_weeks_since_expo, c(4, 49))){
    write.csv(tbl_event_count, paste0("CCU002_DataPreparation/results_phen/tbl_event_count_red_" , expo, "_", event, "_", agegp, "_", pheno_str, ".csv"), row.names = T)
  } else (
    write.csv(tbl_event_count, paste0("CCU002_DataPreparation/results_phen/tbl_event_count_" , expo, "_", event, "_", agegp, "_", pheno_str, ".csv"), row.names = T)
  )
