...
# About:        Make survival data -- format for time-dependent coxph
# ******************************************************************************
...
fit_get_data_surv <- function(covars, agegp, event, survival_data, cuts_weeks_since_expo){
  ...
  # no region in Wales
  #df_age_region <- cohort_agegp %>% dplyr::select(ALF_E, AGE_AT_COHORT_START, region_name) %>% rename(age = AGE_AT_COHORT_START)
  ...
  with_expo %>%
    dplyr::select(ALF_E, expo_date, end_date, event_date, days_to_end, DATE_OF_DEATH) %>%
  ...
  if (identical(cuts_weeks_since_expo, c(4, 49))){
    write.csv(tbl_event_count, paste0("CCU002_DataPreparation/results/tbl_event_count_red_" , expo, "_", event, "_", agegp, ".csv"), row.names = T)
  } else (
    write.csv(tbl_event_count, paste0("CCU002_DataPreparation/results/tbl_event_count_" , expo, "_", event, "_", agegp, ".csv"), row.names = T)
  )

  #===============================================================================
  # FINALIZE age, data_surv
  #-------------------------------------------------------------------------------
